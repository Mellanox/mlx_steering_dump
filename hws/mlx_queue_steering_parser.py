    ...
    try:
        ...
        for ctx in ctxs:
            ...
            if _config_args.get("queue_id") > -1:
                for sr in ctx.send_engine:
                    if int(sr.data["id"]) == _config_args.get("queue_id"):
                        queue_size = int(sr.data["num_entries"])
                        for se in sr.send_ring:
                            output, pi = dr_dump_send_queue(int(se.data["sqn"]) , queue_size,
