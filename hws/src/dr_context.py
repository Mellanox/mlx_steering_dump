    ...
    def tree_print(self, verbosity, tabs, output_file):
        ...
        if verbosity > 3 or self.send_ring_in_error:
            for se in self.send_engine:
                _str = _str + se.tree_print(3, tabs)
...
class dr_parse_context_caps():
    def __init__(self, data):
        ...
        if self.data.get("fw_version") >= f'{_config_args.get("fw_version_major")}.{FW_VERSION_MINOR_FORMAL_RD_SEGMENTS}':
...
class dr_parse_context_send_ring():
    ...
    def dump_str(self, verbosity):
        ...
        if self.wqe_err:
            ret = ret + self.wqe_err.dump_str(4)
        return ret
