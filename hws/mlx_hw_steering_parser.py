...
def parse_args():
    parser = argparse.ArgumentParser(
        description="mlx_hw_steering_parser.py - HW Steering dump tool.",
        epilog="Note: This parser is still under developement, so not all the args are supported yet.",
        add_help=False,
    )
    parser.add_argument(
        "-f", dest="file_path", default="", help="Input steering dump file path."
    )
    parser.add_argument(
        "-v",
        action="count",
        dest="verbose",
        default=0,
        help="Increase output verbosity - v, vv, vvv & vvvv for extra verbosity.",
    )
    parser.add_argument(
        "-skip_hw",
        action="store_false",
        default=True,
        dest="dump_hw_resources",
    ...
    parser.add_argument(
        "-pid", dest="dpdk_pid", type=int, default=-1, help="Trigger DPDK app <PID>."
    )
    parser.add_argument(
        "-port",
        dest="dpdk_port",
...
if __name__ == "__main__":
    try:
        ...
        load_to_db = (
            ...
            else _config_args.get("load_hw_resources")
        )
