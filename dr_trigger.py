...
class msghdr(Structure):
    _fields_ = [('msg_name', c_void_p), ('msg_namelen', c_uint),
	        ('msg_iov', POINTER(iovec)), ('msg_iovlen', c_size_t),
                ('msg_control', c_void_p), ('msg_controllen', c_size_t),
	        ('msg_flags', c_int)]
...
def request_dump(sock, port, dump_file):
    ...
    libc = CDLL('libc.so.6')
    send_fd_msg = libc.sendmsg
    send_fd_msg.argtypes = [c_int, POINTER(msghdr), c_int]
    send_fd_msg.restype = c_int
    ...
    ret = send_fd_msg(sock_num, msg, 0)
    if ret == -1:
